...
class Stripe {
  ...
  convertNativePayTokenToPaymentMethod = tokenId => {
    return Platform.select({
      ios: () => this.convertApplePayTokenToPaymentMethod(tokenId),
      android: () => this.convertAndroidPayTokenToPaymentMethod(tokenId),
    })();
  };

  convertApplePayTokenToPaymentMethod = tokenId => {
    checkInit(this);
    return StripeModule.convertApplePayTokenToPaymentMethod(tokenId);
  };

  convertAndroidPayTokenToPaymentMethod = tokenId => {
    checkInit(this);
    return StripeModule.convertAndroidPayTokenToPaymentMethod(tokenId);
  };
}
